...
(function() {
  window.widgetLoader = (function(window, document) {
    "use strict";
    var $s, addReporterrorListeners, addWidget, addWidgetListeners, assignModal, cssNumber, defaults, elements, error, isMobile, loadModule, make, onSubmitComplete, openModal, submitError, trace;
    defaults = {
    ...
    onSubmitComplete = function(error) {
      var contactBtn, contactResponse, iframe;
    ...
    addReporterrorListeners = function() {
      var contactBtn, iframe;
      iframe = $s('#iframe_widget').elem.contentDocument;
      contactBtn = $s('#btn-submit', iframe);
      contactBtn.elem.addEventListener('click', function(e) {
        var contactEmail, contactNotification, info;
        e.preventDefault();
        ...
        return submitError(info);
      });
    };
    submitError = function(infoObject) {
      var img, src;
